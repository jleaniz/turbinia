...
class TestEvidence(interface.Evidence):
  POSSIBLE_STATES = [interface.EvidenceState.MOUNTED]
...
class TestTurbiniaEvidence(unittest.TestCase):
  ...
  def testEvidenceSerialization(self):
    ...
    rawdisk = raw.RawDisk(name='My Evidence', source_path='/tmp/foo')
    rawdisk_json = rawdisk.to_json()
    self.assertTrue(isinstance(rawdisk_json, str))

    rawdisk_new = interface.evidence_decode(json.loads(rawdisk_json))
    self.assertIsInstance(rawdisk_new, interface.RawDisk)
  ...
  def testEvidenceCollectionDeserialization(self):
    ...
    rawdisk = raw.RawDisk(name='My Evidence', source_path='/tmp/foo.img')
    collection = interface.EvidenceCollection()
    ...
    collection_new = interface.EvidenceManager.evidence_decode(
    ...
    self.assertIsInstance(collection_new, interface.EvidenceCollection)
    self.assertIsInstance(rawdisk_new, interface.RawDisk)
  ...
  def testEvidenceCollectionSerialization(self):
    ...
    evidence_ = interface.EvidenceCollection()
    rawdisk = interface.RawDisk(name='My Evidence', source_path='/tmp/foo.img')
  ...
  def testEvidenceSerializationBadType(self):
    ...
    self.assertRaises(
        TurbiniaException, interface.EvidenceManager.evidence_decode, [1, 2])

  def testEvidenceSerializationNoTypeAttribute(self):
    """Test that evidence_decode throws error on dict with no type attribute."""
    test = {1: 2, 3: 4}
    self.assertRaises(
        TurbiniaException, interface.EvidenceManager.evidence_decode, test)
  ...
  def testEvidenceValidationEmptyAttribute(self):
    ...
    rawdisk = interface.RawDisk(name='My Evidence', source_path=None)
    rawdisk.REQUIRED_ATTRIBUTES = ['name', 'source_path']
  ...
  def testEvidencePreprocess(self, mock_preprocess):
    ...
    test_evidence.preprocess(
        'task123', required_states=[interface.EvidenceState.ATTACHED])
    mock_preprocess.assert_called_with(None, [interface.EvidenceState.ATTACHED])
